...
contract ProvenanceStorage is ProvenanceCore{
    ...
    {
        // the index of provenance record is bigger than the index list => provenance record does not exists
    ...
    function getProvenance(uint _provId) public view returns (uint tokenId, string memory context, uint index)
    {
    ...
    function _createProvenance(uint _provId, uint _tokenId, string memory _context) internal returns (uint index) {
    ...
    function _updateProvenance(uint _provId, uint _tokenId, string memory _context) internal returns (bool success) {
        require(isProvenance(_provId), "Non-existent key");
